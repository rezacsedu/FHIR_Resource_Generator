...
class Patient(object):
    ...
    def generate(cls, patient_file_name=RI_PATIENTS_FILE):
        ...
        pats = csv.reader(f, dialect='excel-tab')
        header = next(pats)
        f.close()
        fw = open(PATIENTS_FILE, 'w')
        # Read in patient data:
        for pat in range(1,20000):
...
##            print(header)
            ...
            p['GENDER'] = 'M' if randint(0, 1) else 'F'
##            p['YOB'] = '20' if randint(0, 1) else '19'
            year = randint(1950, 2019)
            p['YOB'] = str(year)
            p['RACE'] = 'White' if randint(0, 1) else 'Black'
            p['PID'] = str(randint(1, 999999))
            p['gp'] = str(randint(1, 999999))
            ...
            adr = rndAddress()
            p = dict(list(p.items()) + list(adr.items()))
            ...
            if top:
                head = p.keys()
                print ("\t".join(head), file=fw)
                top = False
            # Then write out the row:
            print  ("\t".join([p[field] for field in head]), file=fw)
        fw.close()
